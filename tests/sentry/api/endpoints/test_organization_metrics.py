...
from sentry.snuba.metrics.fields.snql import _percentage_in_snql
...
class DerivedMetricsDataTest(SessionMetricsTestCase, APITestCase):
    ...
    @with_feature(FEATURE_FLAG)
    @mock.patch(
        "sentry.snuba.metrics.query_builder.DERIVED_METRICS",
        {
    ...
    @mock.patch(
        "sentry.snuba.metrics.fields.base.DERIVED_METRICS",
        {
            **ORIGINAL_DERIVED_METRICS,
            "crash_free_fake": SingularEntityDerivedMetric(
                metric_name="crash_free_fake",
